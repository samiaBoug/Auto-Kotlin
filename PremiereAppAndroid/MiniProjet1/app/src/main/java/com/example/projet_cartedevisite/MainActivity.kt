...
fun CarteDeVisite() {
    ...
    Column(
        ...
        val callIntent = Intent(Intent.ACTION_DIAL).apply {
            data =Uri.parse("tel:+212666666666")
        }
        ContactInfo(
            label = "Téléphone :",
            value = "+212 6 12 34 56 78",
            onClick = { context.startActivity(callIntent)/* Appeler */ }
        )
        val emailIntent = Intent(Intent.ACTION_SENDTO).apply {
            data = Uri.parse("mailto:bougtoubsamia@gmail.com")
        }
        ContactInfo(
            label = "Email :",
            value = "bougtoubsamia@gmail.com",
            onClick = { context.startActivity((emailIntent)) /* Envoyer un email */ }
        )
        Spacer(modifier = Modifier.height(16.dp))

        // Bouton pour site web
        val webSiteIntent = Intent(Intent.ACTION_VIEW).apply{
            data = Uri.parse("http://www.bougtoubsamia.com")
        }
